...
# --- 1. KONFIGURASI & SETUP ---
# ==========================================
st.set_page_config(layout="wide", page_title="Indodax Scalper Pro V8.6")
...
def get_orderbook_analysis(symbol):
    try:
        exchange = ccxt.indodax()
        # Ambil 50 antrian teratas (Deep Scan)
...
def process_indicators(df):
    ...
    # FIX: Isi nilai ATR awal yang kosong agar Supertrend tidak putus
    df['ATR'] = df['ATR'].bfill()
    ...
    df['supertrend'] = df['st_upper'] # Default awal
    df['st_dir'] = 1 
...
# --- 6. DASHBOARD UTAMA ---
# ==========================================
st.sidebar.header("🎛️ Scalper V8.6")
...
def dashboard(sym, tf):
    ...
    # --- HTML LAYOUT (DIKEMBALIKAN SESUAI PERMINTAAN) ---
    st.markdown(f"""
    <style>
    ...
    # --- PLOT CHART (ZOOM FIXED) ---
    # Range Zoom: Default 60 candle terakhir
    ...
    # 2. Supertrend Line (Dengan Filtering Data)
    st_bull = df.copy()
    ...
    # --- LAYOUT SETTINGS (KUNCI ZOOM & SLIDER) ---
    fig.update_layout(
        height=650, 
        template="plotly_dark", 
        margin=dict(l=0,r=50,t=0,b=0), 
        xaxis=dict(
            range=[range_start, range_end], # Default view 60 candle
            rangeslider=dict(visible=True), # Slider aktif
            type="date"
        ),
