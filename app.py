...
def show_dashboard(sym, tf):
    try:
        ...
        # Konversi data ke float agar tidak error formatting
        curr = float(ticker['last'])
        high_24h = float(ticker['high'])
        low_24h = float(ticker['low'])
        vol = float(ticker['baseVolume'])
        
        # Logika Sinyal & Prediksi
        ...
        # Format Angka Indonesia (Titik pemisah ribuan)
        def fmt(x): return f"{x:,.0f}".replace(",", ".")

        # --- STYLE CSS ---
        st.markdown(f"""
        <style>
            .main-container {{ display: flex; gap: 10px; flex-wrap: wrap; }}
            .box-left {{ 
                flex: 1; background-color: {sig_bg}; border: 1px solid {sig_col}; 
            ...
            .label {{ font-size: 11px; color: #999; font-weight: bold; margin-bottom: 5px; }}
            .val-price {{ font-size: 18px; font-weight: bold; color: #f1c40f; }}
            .val-hl {{ font-size: 14px; font-weight: bold; }}
            .sig-title {{ font-size: 22px; font-weight: 900; color: {sig_col}; margin-bottom: 5px; }}
            .pred-grid {{ display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 5px; margin-top: 10px; border-top: 1px solid #444; padding-top: 10px; }}
        </style>
        ...
        <div class="main-container">
            ...
            <div class="box-left">
                ...
                <div class="sig-title">{sig_text}</div>
                <div class="pred-grid">
                    <div class="inner-stat"><div class="label">ENTRY</div><div style="font-size:12px; color:white;">{fmt(entry)}</div></div>
                    <div class="inner-stat"><div class="label">TARGET</div><div style="font-size:12px; color:#00ff00;">{fmt(tp)}</div></div>
                    <div class="inner-stat"><div class="label">S. LOSS</div><div style="font-size:12px; color:#ff4444;">{fmt(sl)}</div></div>
                </div>
            </div>
